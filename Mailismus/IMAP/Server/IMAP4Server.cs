...
namespace com.grey.mailismus.imap.server
{
		...
		// com.grey.mailismus.ms.maildir.MailboxSession.UpdatesListener
	{
		...
		{
			private readonly IMAP4Server prototype;
	...
	private static readonly bool UNILATERAL_REPORTS = SysProps.get("grey.imap.unilateral_reports", true);
	private static readonly bool REPORT_ON_APPEND = SysProps.get("grey.imap.report_on_append", true);
	private static readonly bool REPORT_ON_EXPUNGE = SysProps.get("grey.imap.report_on_expunge", true);
	private static readonly bool REPORT_NEWFLAGS = SysProps.get("grey.imap.report_newflags", true);
	private static readonly bool REPORT_UID = SysProps.get("grey.imap.report_uid", false);
	private static readonly bool IGNORE_SET_RECENT = SysProps.get("grey.imap.ignore_set_recent", false);
	private static readonly bool IMPLICIT_TEXT = SysProps.get("grey.imap.assumetext", true);
	private static readonly bool SQUIRREL_FRIENDLY = SysProps.get("grey.imap.squirrelmail", true);

	private static readonly class FSM_Trigger
	{
		readonly PROTO_STATE state;
		readonly PROTO_EVENT evt;
		readonly com.grey.@base.utils.ByteChars cmd;
		readonly PROTO_STATE new_state;
	...
	private static readonly FSM_Trigger[] fsmTriggers = {
	...
	private static readonly byte[] FETCHMACRO_FAST = "FLAGS INTERNALDATE RFC822.SIZE".getBytes();
	private static readonly byte[] FETCHMACRO_ALL = "FLAGS INTERNALDATE RFC822.SIZE ENVELOPE".getBytes();
	private static readonly byte[] FETCHMACRO_FULL = "FLAGS INTERNALDATE RFC822.SIZE ENVELOPE BODY".getBytes();
	...
	private static readonly SearchKey[] searchKeys = new SearchKey[] {new SearchKey("charset", true, 1),
	...
	private static readonly int TMRTYPE_SESSIONTMT = 1;
	private static readonly int TMRTYPE_DISCON = 2;
	private static readonly int TMRTYPE_BULKCMD = 3;
	private static readonly int TMRTYPE_NEWMAIL = 4;

	//state2 flags
	private static readonly int S2_ENDED = 1 << 0; //we have already called endConnection()
	private static readonly int S2_REQWAIT = 1 << 1; //we're waiting for the remote client to send a request
	private static readonly int S2_DATAWAIT = 1 << 2; //we're in the middle of receiving a data stream
	private static readonly int S2_NOTRANSCRIPT = 1 << 3;

	// flags arg to getNextTerm()
	private static readonly int TERM_EMPTY = 1 << 0; //blank term is ok
	private static readonly int TERM_COPY = 1 << 1;

	private readonly SharedFields shared;
	private readonly com.grey.@base.utils.TSAP remote_tsap = new com.grey.@base.utils.TSAP();
	private readonly com.grey.@base.utils.ByteChars reqtag = new com.grey.@base.utils.ByteChars();
	private readonly java.io.File append_fh; //temp staging file into which we stream an Append message
	...
	private readonly BulkCommand.CommandFetch cmdFetch;
	private readonly BulkCommand.CommandStore cmdStore;
	private readonly BulkCommand.CommandCopy cmdCopy;
	private readonly BulkCommand.CommandSearch cmdSearch;
	private readonly NumberList bulkseqlst = new NumberList();
	...
	private bool isState2(int f) { return Defs.isFlagSet(state2, f); }
				...
				throws java.io.IOException
	{
		...
		public void ioReceived(ByteArrayRef rcvdata) throws java.io.IOException
		{
			...
			bool ok = getNextTerm(rcvdata, reqtag, TERM_COPY);
		...
		private PROTO_STATE issueResponse(bool ok, PROTO_EVENT errtype, PROTO_STATE errstate, CharSequence diagnostic)
					throws java.io.IOException
		{
				if (ok) {
		...
		private void handleBulkCommand(PROTO_EVENT evt, bool uidmode, PROTO_STATE prevstate, ByteArrayRef rcvdata) throws java.io.IOException
		{
			PROTO_EVENT errtype = PROTO_EVENT.E_BADCMD;
			String errmsg = "Invalid arguments";
			bool ok;
		...
		@Override
			protected void disconnectLingerDone(bool ok, CharSequence info, Throwable ex)
		{
		...
		private void transmit(java.nio.ByteBuffer xmtbuf, bool finalrsp) throws java.io.IOException
		{
		...
		private void transmit(com.grey.@base.utils.ByteChars data, bool finalrsp) throws java.io.IOException
		{
		...
		private void processLogin(bool is_valid, com.grey.@base.utils.ByteChars usrnam) throws java.io.IOException
		{
				if (!is_valid) {
				transitionState(PROTO_STATE.S_AUTH);
				...
				throws java.io.IOException
		{
			...
			bool wild_partial = false;
			bool wild_full = false;
			...
			bool found_matches = false;
				...
				readonly String mbx = sess.getMailboxName(idx);
				...
				bool is_parent = (idx != mbxcnt - 1 && sess.getMailboxName(idx + 1).startsWith(mbx));
		...
		private String execSelect(bool rdonly, CharSequence mbxname, com.grey.@base.utils.ByteChars xmtbuf)
					throws java.io.IOException
		{
		...
		private void execClose(bool expunge) throws java.io.IOException
		{
				if (expunge && sess.writeableMailbox()) sess.expungeMailbox(null, null);
			sess.closeMailbox();
		}

		private String execFetch(bool uidmode, com.grey.@base.utils.ByteChars seq, com.grey.@base.utils.ByteChars attrs)
				throws java.io.IOException
		{
			...
			bool add_uid = (uidmode || REPORT_UID);
		...
		return null;
			...
			private String execStore(bool uidmode, com.grey.@base.utils.ByteChars seq, com.grey.@base.utils.ByteChars op,
		...
		private String execCopy(bool uidmode, com.grey.@base.utils.ByteChars seq, CharSequence dest_mbxname)
					throws java.io.IOException
		{
			NumberList seqlst = parseSequenceSet(seq, bulkseqlst, uidmode, false);
				if (seqlst == null) return "Invalid sequence-set";
					...
					throws java.io.IOException
		{
			...
			append_flags = msflags.toString();
				...
				// we don't issue a readonly response yet, so prepare to receive the message
		...
		private String execSearch(bool uidmode, com.grey.@base.utils.ByteChars expr, com.grey.@base.utils.ByteChars termbuf,
				...
				throws java.io.IOException
		{
			...
			bool idsearch = false;
			bool have_srchterms = false;
				long mintime = 0;
				long maxtime = 0;
				int minsize = 0;
				int maxsize = 0;

				// We don't handle any bool expressions, or any operators other than not, so any
		...
		private void execBulkCommand(BulkCommand cmd) throws java.io.IOException
		{
			...
			cmd.report_at_end = reportUpdates(xmtbuf, MaildirStore.RPT_FLAGS_ONLY);
				...
				bool final_batch = (msglmt == cmd.seqlst.size());
				String errmsg = null;
		...
		{
			readonly com.grey.mailismus.ms.maildir.MailboxView mbxview = sess.currentView();
			readonly int opslmt = cmd.ops.size();
				...
				readonly int seqnum = cmd.seqlst.get(msgidx);
				readonly bool initial_seen = mbxview.hasFlag(seqnum, MaildirStore.MSGFLAG_SEEN);
				...
				{
					readonly FetchOp op = cmd.ops.get(opidx);
					readonly int prevlen = xmtbuf.size();
					...
					switch (opcode)
					{
						...
						case MIME:
							databuf.clear();
							bool excl = op.def.isSet(FetchOpDef.F_EXCL) || !op.def.isSet(FetchOpDef.F_HASFLDS); //return all headers if !HASFLDS
		...
		private bool execBulkSearch(BulkCommand.CommandSearch cmd, int msglmt, com.grey.@base.utils.ByteChars xmtbuf)
		{
			bool done = sess.searchMessages(cmd.results, cmd.uidmode, cmd.seqlst,
		...
		private bool reportUpdates(com.grey.@base.utils.ByteChars xmtbuf, int opts)
					throws java.io.IOException
		{
			reportMailboxFlags(xmtbuf, REPORT_NEWFLAGS);
			bool have_more = sess.loadUpdates(this, xmtbuf, opts);
			com.grey.mailismus.ms.maildir.MailboxView props = sess.currentView();
		...
		mbxprop_msgtotal--;
			...
			@Override
			public void reportMessageFlags(int seqnum, CharSequence msflags, bool with_uid, Object arg)
		{
			com.grey.@base.utils.ByteChars xmtbuf = (com.grey.@base.utils.ByteChars)arg;
		...
		xmtbuf.append(')').append(IMAP4Protocol.EOL);
			...
			private void reportMailboxFlags(com.grey.@base.utils.ByteChars xmtbuf, bool report) throws java.io.IOException
		{
				int numflags = shared.msgFlags.getNumFlags();
				if (!report || numflags == mbxprop_numflags) return;
			mbxprop_numflags = numflags;
			bool send = (xmtbuf == null);
		...
		private bool getEnvelope(int seqnum, MimePart mime, com.grey.@base.utils.ByteChars outbuf, StringBuilder tmpsb) throws java.io.IOException
		{
			shared.tmpstrmap.clear();
			String []
			hdrs = shared.envHeaderNames;
		...
		private void formatMimeStructure(MimePart mime, bool withExtensions, int seqnum, com.grey.@base.utils.ByteChars outbuf,
				StringBuilder tmpsb) throws java.io.IOException
		{
			outbuf.append('(');
		...
		{
			readonly int pos_open = emaddr.indexOf('<');
			readonly int pos_close = (pos_open == -1 ? -1 : emaddr.indexOf('>', pos_open));
			readonly int pos_dlm = emaddr.indexOf(com.grey.@base.utils.EmailAddress.DLM_DOM, pos_open == -1 ? 0 : pos_open);
			int pos1 = 0;
			int pos2 = 0;
		...
		private String parseFlags(com.grey.@base.utils.ByteChars imapflags, StringBuilder msflags,
				bool create) throws java.io.IOException
		{
				int pos = 0;
				int pos2;
				do {
		...
		private bool parseSequenceRange(com.grey.@base.utils.ByteChars spec, int off, int lmt, NumberList lst,
				bool uidmode, bool srchmode, int msgcnt)
		{
		...
		private int parseSequenceNumber(com.grey.@base.utils.ByteChars spec, int off, int lmt, bool uidmode, int msgcnt)
		{
			if (spec.charAt(off) == '*')
			{
		...
		private static bool matchesCommand(com.grey.@base.utils.ByteChars reqcmd, com.grey.@base.utils.ByteChars targetcmd)
		{
			readonly int cmdlen = targetcmd.size();
		...
		private static bool getNextTerm(ByteArrayRef line, com.grey.@base.utils.ByteChars term, int flags)
		{
		...
		private static bool isNonSynchLiteral(CharSequence octetcnt)
		{
		...
		private static void stripEOL(ByteArrayRef rcvdata, bool trailspace)
		{
